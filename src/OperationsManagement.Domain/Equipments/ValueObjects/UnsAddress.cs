using OperationsManagement.Domain._Shared;

namespace OperationsManagement.Domain.Equipments.ValueObjects;

public sealed class UnsAddress(string enterprise, string site, string area, string processCell, string asset) : ValueObject
{
    public string Enterprise { get; init; } = enterprise;   // Level 4/5 root
    public string Site { get; init; } = site;
    public string Area { get; init; } = area;
    public string ProcessCell { get; init; } = processCell; // ISA-88 "ProcessCell/Line"
    public string Asset { get; init; } = asset;             // ISA-88 "Unit/Machine"

    public static Result<UnsAddress> Create(string enterprise, string site, string area, string processCell, string asset)
    {
        var unsPath = new UnsAddress(enterprise, site, area, processCell, asset);

        return unsPath;
    }

    // Formats the path for MQTT topics
    public string ToTopic() => $"{Site}/{Area}/{ProcessCell}/{Asset}";

    // Formats for the Command branch
    public string ToCommandTopic(string command) => $"{ToTopic()}/Cmd/{command}";

    protected override IEnumerable<object> GetAtomicValues()
    {
        throw new NotImplementedException();
    }
}
